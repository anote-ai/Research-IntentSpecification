# DEPRECATED: System A scoring functions.
# Retained so existing imports don't break. New code should use execute.py and ivr.py.
...
def score_intent_alignment(spec: "IntentSpec", call: "ToolCall") -> float:
    ...
    if not spec.constraints:
        ...
        matched = sum(1 for c in spec.constraints if c.lower() in rationale_lower)
...
def constraint_satisfaction_rate(spec: "IntentSpec", call: "ToolCall") -> float:
    if not spec.constraints:
        return 1.0
    rationale_lower = call.rationale.lower()
    satisfied = sum(1 for c in spec.constraints if c.lower() in rationale_lower)
...
def ambiguity_resolution_score(spec: "IntentSpec", call: "ToolCall") -> float:
    ...
    resolved = sum(1 for a in spec.ambiguities if a.lower() in searchable)
...
def compare_scores(intent_scores: list[float], syntactic_scores: list[float]) -> dict:
    n = len(intent_scores)
    if n == 0:
    ...
    return {"pearson_r": r, "mean_divergence": sum(divergences) / n, "cases_where_syntactic_overstates": overstates}


def divergence_analysis(records: list["EvalRecord"]) -> dict:
    if not records:
        return {
            "mean_intent": 0.0, "mean_syntactic": 0.0, "mean_divergence": 0.0,
            "max_divergence": 0.0, "overstatement_rate": 0.0,
            "mean_constraint_sat_rate": 0.0, "mean_ambiguity_res_score": 0.0,
        }
    n = len(records)
    ...
    return {
        ...
        "mean_constraint_sat_rate": sum(r.constraint_sat_rate for r in records) / n,
        "mean_ambiguity_res_score": sum(r.ambiguity_res_score for r in records) / n,
    }
