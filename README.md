...
- Across all 245 generated solutions (49 problems × 5 samples), the stated
  test **C1 passes 94.3%** of the time (14 failures). Hidden constraints fail
  far more often: **C2 42.9%** (99/231), **C3 28.9%** (58/201), **C4 15.4%**
  (10/65) — computed over the solutions whose problem actually included that
  constraint (not every problem has a C3 or C4), matching the paper's
  Table 1.
...
- `results/experiment1.json` records, per task and per solution, whether the
  ...
  failed — this is what the per-constraint fail rates above are derived
  from.
- `python scripts/plot_ivr_distribution.py [results/experiment1.json]`
  ...
  the input file. Note this produces a PNG; the `ivr_distribution.pdf` figure
  embedded in the paper was exported separately and is not regenerated by
  this script as-is.
